...
namespace BlueStacks.hyperDroid.Common
{
	public class JsonParser
	{
		...
		public static bool IsAppInstalled(string packageName, out string version)
		{
			JsonParser.GetAppList();
			for (int i = 0; i < JsonParser.s_originalJson.Length; i++)
			{
				if (JsonParser.s_originalJson[i].package == packageName)
				{
					version = JsonParser.s_originalJson[i].version;
					return true;
				}
			}
			version = "NA";
		...
		public static void WriteJson(AppInfo[] json)
		{
			...
			for (int i = 0; i < json.Length; i++)
			{
				...
				jSonWriter.WriteMember("name", json[i].name);
				...
				jSonWriter.WriteMember("version", json[i].version);
				if (json[i].url != null)
				{
